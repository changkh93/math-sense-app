...
**파이썬 수학 → 함수와 클래스 기초 (11 ~ 20) → for 중첩문, 별찍기, 구구단 출력**에서 가져온 ‘세트 2: range와 if로 짝수 리스트 만들기’ 코드입니다. 단원 전체를 요약하는 대신 이 단계가 담당하는 작업을 한 가지씩 읽습니다. 코드 속 이름·점수·예시 자료는 교재의 연습 데이터이며 실제 학생의 기록이나 성과를 소개하는 자료가 아닙니다.
...
- **1행**: `even_nums`에 `[]`의 값을 저장합니다.
- **3행**: `range(1, 21)`에서 값을 하나씩 꺼내 `i`로 읽습니다. 들여쓴 본문이 반복 범위입니다.
- **4행**: `i % 2 == 0`를 검사합니다. 참인 경로와 그렇지 않은 경로에서 바뀌는 값을 나누어 보세요.
- **5행**: `even_nums.append(i)`를 호출합니다. 호출 앞뒤로 대상의 상태가 달라지는지 살펴보세요.
- **7행**: `print(even_nums)`를 호출합니다. 괄호 안의 값을 화면 출력과 연결해 읽으세요.
- **9행**: `list(range(3, 21, 3))`를 호출합니다. 호출 앞뒤로 대상의 상태가 달라지는지 살펴보세요.
