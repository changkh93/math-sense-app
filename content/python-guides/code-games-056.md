...
**게임 프로젝트 → 화성 탐사대 I · 고급 Pygame 기초 → 타일을 조립해 지형 표시하기**에서 가져온 ‘캐시에 있는 그림 다시 사용하기’ 코드입니다. 단원 전체를 요약하는 대신 이 단계가 담당하는 작업을 한 가지씩 읽습니다. 코드 속 이름·점수·예시 자료는 교재의 연습 데이터이며 실제 학생의 기록이나 성과를 소개하는 자료가 아닙니다.
...
- **1행**: `load_image(path, size)` 함수를 정의합니다. 정의와 실제 호출 시점은 구분합니다.
- **2행**: `key`에 `(path, size)`의 값을 저장합니다.
- **3행**: `key not in image_cache`를 검사합니다. 참인 경로와 그렇지 않은 경로에서 바뀌는 값을 나누어 보세요.
- **4행**: `image_cache[key]`에 `pygame.transform.smoothscale(pygame.image.load(path).convert_alpha(), size)`의 값을 저장합니다.
