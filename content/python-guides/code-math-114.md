...
**파이썬 수학 → 수학 알고리즘 (31 ~ 40) → 유클리드호제법**에서 가져온 ‘세트 3: 한 줄 변수 교환으로 더 간단한 gcd 함수 만들기’ 코드입니다. 단원 전체를 요약하는 대신 이 단계가 담당하는 작업을 한 가지씩 읽습니다. 코드 속 이름·점수·예시 자료는 교재의 연습 데이터이며 실제 학생의 기록이나 성과를 소개하는 자료가 아닙니다.
...
- **7행**: `print(gcd(24, 10))`를 호출합니다. 괄호 안의 값을 화면 출력과 연결해 읽으세요.
- **8행**: `print(gcd(100, 25))`를 호출합니다. 괄호 안의 값을 화면 출력과 연결해 읽으세요.
- **9행**: `print(gcd(81, 27))`를 호출합니다. 괄호 안의 값을 화면 출력과 연결해 읽으세요.
