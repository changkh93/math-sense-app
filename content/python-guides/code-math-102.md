...
**파이썬 수학 → 수학 알고리즘 (31 ~ 40) → 소수_판별_함수**에서 가져온 ‘세트 3: 제곱근까지만 확인하는 효율적인 소수 판별’ 코드입니다. 단원 전체를 요약하는 대신 이 단계가 담당하는 작업을 한 가지씩 읽습니다. 코드 속 이름·점수·예시 자료는 교재의 연습 데이터이며 실제 학생의 기록이나 성과를 소개하는 자료가 아닙니다.
...
- **11행**: `print(is_prime(11))`를 호출합니다. 괄호 안의 값을 화면 출력과 연결해 읽으세요.
- **12행**: `print(is_prime(36))`를 호출합니다. 괄호 안의 값을 화면 출력과 연결해 읽으세요.
