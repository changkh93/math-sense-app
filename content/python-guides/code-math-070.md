...
**파이썬 수학 → 함수와 클래스 기초 (11 ~ 20) → _args**에서 가져온 ‘세트 5: 자릿수를 돌려주는 함수 만들기’ 코드입니다. 단원 전체를 요약하는 대신 이 단계가 담당하는 작업을 한 가지씩 읽습니다. 코드 속 이름·점수·예시 자료는 교재의 연습 데이터이며 실제 학생의 기록이나 성과를 소개하는 자료가 아닙니다.
...
print(f'{a}의 자릿수와 {b}의 자릿수의 합은? {total_digits}')
```

아래 행 번호는 위 코드 블록의 첫 줄을 1행으로 셉니다. 긴 예제에서는 주요 문장 10개까지 짚었습니다. 함수 안의 줄은 함수를 호출할 때 실행되므로, 행 번호 순서와 실제 실행 순서가 항상 같은 것은 아닙니다.

- **1행**: `digit_check(n)` 함수를 정의합니다. 정의와 실제 호출 시점은 구분합니다.
- **2행**: `result`에 `len(str(n))`의 값을 저장합니다.
- **3행**: `result`을 호출한 곳으로 돌려주고 이번 함수 실행을 끝냅니다.
- **5행**: `a`에 `1234510`의 값을 저장합니다.
- **6행**: `b`에 `44393`의 값을 저장합니다.
- **8행**: `total_digits`에 `digit_check(a) + digit_check(b)`의 값을 저장합니다.
- **9행**: `print(f'{a}의 자릿수와 {b}의 자릿수의 합은? {total_digits}')`를 호출합니다. 괄호 안의 값을 화면 출력과 연결해 읽으세요.
...
```text
1234510의 자릿수와 44393의 자릿수의 합은? 12
```
