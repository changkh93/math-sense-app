...
**파이썬 수학 → 수학 알고리즘 (31 ~ 40) → 최소공배수**에서 가져온 ‘세트 1: 배수 목록으로 최소공배수 찾기’ 코드입니다. 단원 전체를 요약하는 대신 이 단계가 담당하는 작업을 한 가지씩 읽습니다. 코드 속 이름·점수·예시 자료는 교재의 연습 데이터이며 실제 학생의 기록이나 성과를 소개하는 자료가 아닙니다.

학습 목표는 코드를 그대로 입력하는 데서 멈추지 않고, 어떤 값이 준비되고 어떤 조건에서 결과가 달라지는지를 설명하는 것입니다. 다음 질문에 먼저 답을 적고 코드와 대조해 보세요.

> 조건 `i in b_multiples`가 참일 때와 거짓일 때, 다음에 실행되는 줄은 무엇인가요?
...
print(f'20과 75의 최소공배수 방법1: {result_lcm}')
```

아래 행 번호는 위 코드 블록의 첫 줄을 1행으로 셉니다. 긴 예제에서는 주요 문장 10개까지 짚었습니다. 함수 안의 줄은 함수를 호출할 때 실행되므로, 행 번호 순서와 실제 실행 순서가 항상 같은 것은 아닙니다.

- **1행**: `lcm(a, b)` 함수를 정의합니다. 정의와 실제 호출 시점은 구분합니다.
- **2행**: `a_multiples`에 `range(a, a * b + 1, a)`의 값을 저장합니다.
- **3행**: `b_multiples`에 `range(b, a * b + 1, b)`의 값을 저장합니다.
- **5행**: `a_multiples`에서 값을 하나씩 꺼내 `i`로 읽습니다. 들여쓴 본문이 반복 범위입니다.
- **6행**: `i in b_multiples`를 검사합니다. 참인 경로와 그렇지 않은 경로에서 바뀌는 값을 나누어 보세요.
- **7행**: `i`을 호출한 곳으로 돌려주고 이번 함수 실행을 끝냅니다.
- **9행**: `a * b`을 호출한 곳으로 돌려주고 이번 함수 실행을 끝냅니다.
- **11행**: `result_lcm`에 `lcm(20, 75)`의 값을 저장합니다.
- **12행**: `print(f'20과 75의 최소공배수 방법1: {result_lcm}')`를 호출합니다. 괄호 안의 값을 화면 출력과 연결해 읽으세요.
...
이 글의 확인 질문은 **조건 `i in b_multiples`가 참일 때와 거짓일 때, 다음에 실행되는 줄은 무엇인가요?** 입니다. 보호자는 정답을 먼저 알려주기보다 아이가 실제 변수나 조건을 짚어 설명하는지 들어보세요. 어려워하면 단원 전체를 다시 시키기보다 위 핵심 줄 중 막힌 한 줄로 범위를 좁힙니다.
