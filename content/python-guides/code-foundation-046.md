...
**처음 파이썬 → 21 ~ 27 → if문 활용하여 도형그리기**에서 가져온 ‘비교 결과를 바로 return하기’ 코드입니다. 단원 전체를 요약하는 대신 이 단계가 담당하는 작업을 한 가지씩 읽습니다. 코드 속 이름·점수·예시 자료는 교재의 연습 데이터이며 실제 학생의 기록이나 성과를 소개하는 자료가 아닙니다.
...
def is_even_simple(num):
    return num % 2 == 0
print(is_even_simple(20))
print(is_even_simple(17))
```

아래 행 번호는 위 코드 블록의 첫 줄을 1행으로 셉니다. 긴 예제에서는 주요 문장 10개까지 짚었습니다. 함수 안의 줄은 함수를 호출할 때 실행되므로, 행 번호 순서와 실제 실행 순서가 항상 같은 것은 아닙니다.

- **1행**: `is_even_simple(num)` 함수를 정의합니다. 정의와 실제 호출 시점은 구분합니다.
- **2행**: `num % 2 == 0`을 호출한 곳으로 돌려주고 이번 함수 실행을 끝냅니다.
- **3행**: `print(is_even_simple(20))`를 호출합니다. 괄호 안의 값을 화면 출력과 연결해 읽으세요.
- **4행**: `print(is_even_simple(17))`를 호출합니다. 괄호 안의 값을 화면 출력과 연결해 읽으세요.
