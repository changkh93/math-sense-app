...
**게임 프로젝트 → 기초 (1 ~ 10) → pygame 시작, window 설정**에서 가져온 ‘Pygame 시작과 종료하기’ 코드입니다. 단원 전체를 요약하는 대신 이 단계가 담당하는 작업을 한 가지씩 읽습니다. 코드 속 이름·점수·예시 자료는 교재의 연습 데이터이며 실제 학생의 기록이나 성과를 소개하는 자료가 아닙니다.
...
- **3행**: `print('Pygame 로봇, 성공적으로 깨어났습니다!')`를 호출합니다. 괄호 안의 값을 화면 출력과 연결해 읽으세요.
- **4행**: `pygame.quit()`를 호출합니다. 호출 앞뒤로 대상의 상태가 달라지는지 살펴보세요.
- **5행**: `print('Pygame 로봇, 안전하게 잠들었습니다!')`를 호출합니다. 괄호 안의 값을 화면 출력과 연결해 읽으세요.
