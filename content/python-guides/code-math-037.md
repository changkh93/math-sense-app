...
**파이썬 수학 → 기초 자료형과 제어문 (1 ~ 10) → for반복문**에서 가져온 ‘세트 4: for 반복문으로 총합 구하기’ 코드입니다. 단원 전체를 요약하는 대신 이 단계가 담당하는 작업을 한 가지씩 읽습니다. 코드 속 이름·점수·예시 자료는 교재의 연습 데이터이며 실제 학생의 기록이나 성과를 소개하는 자료가 아닙니다.
...
- **5행**: `total`에 `total + i`의 값을 저장합니다.
- **7행**: `print('총 합계는?', total)`를 호출합니다. 괄호 안의 값을 화면 출력과 연결해 읽으세요.
