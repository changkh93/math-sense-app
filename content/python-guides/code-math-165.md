...
**파이썬 수학 → 방정식과 시각화 (51 ~ 60) → 부등식 기초**에서 가져온 ‘세트 4: 넘파이 배열에 람다 조건 적용하기’ 코드입니다. 단원 전체를 요약하는 대신 이 단계가 담당하는 작업을 한 가지씩 읽습니다. 코드 속 이름·점수·예시 자료는 교재의 연습 데이터이며 실제 학생의 기록이나 성과를 소개하는 자료가 아닙니다.
...
- **3행**: `x`에 `np.arange(-10, 11)`의 값을 저장합니다.
- **4행**: `b`에 `lambda a: a + 5 >= 4`의 값을 저장합니다.
- **5행**: `b(x)`를 호출합니다. 호출 앞뒤로 대상의 상태가 달라지는지 살펴보세요.
