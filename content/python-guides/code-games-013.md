...
**게임 프로젝트 → 기초 (1 ~ 10) → mouse movement**에서 가져온 ‘드래곤의 처음 위치 정하기’ 코드입니다. 단원 전체를 요약하는 대신 이 단계가 담당하는 작업을 한 가지씩 읽습니다. 코드 속 이름·점수·예시 자료는 교재의 연습 데이터이며 실제 학생의 기록이나 성과를 소개하는 자료가 아닙니다.
...
- **2행**: `dragon_rect`에 `dragon_img.get_rect()`의 값을 저장합니다.
- **3행**: `dragon_rect.topleft`에 `(25, 25)`의 값을 저장합니다.
