...
**파이썬 수학 → 클래스와 배열 (41 ~ 50) → list내포**에서 가져온 ‘세트 1: def와 lambda 함수 비교하기’ 코드입니다. 단원 전체를 요약하는 대신 이 단계가 담당하는 작업을 한 가지씩 읽습니다. 코드 속 이름·점수·예시 자료는 교재의 연습 데이터이며 실제 학생의 기록이나 성과를 소개하는 자료가 아닙니다.
...
- **1행**: `sum_num(a, b)` 함수를 정의합니다. 정의와 실제 호출 시점은 구분합니다.
- **2행**: `a + b`을 호출한 곳으로 돌려주고 이번 함수 실행을 끝냅니다.
- **4행**: `print(f'def로 만든 함수 결과: {sum_num(10, 3)}')`를 호출합니다. 괄호 안의 값을 화면 출력과 연결해 읽으세요.
- **6행**: `sums`에 `lambda x, y: x + y`의 값을 저장합니다.
- **7행**: `print(f'lambda 함수 결과: {sums(2, 6)}')`를 호출합니다. 괄호 안의 값을 화면 출력과 연결해 읽으세요.
- **9행**: `print(f'이름 없는 lambda 결과: {(lambda x, y: x + y)(5, 9)}')`를 호출합니다. 괄호 안의 값을 화면 출력과 연결해 읽으세요.
- **11행**: `is_even`에 `lambda x: x % 2 == 0`의 값을 저장합니다.
- **12행**: `print(f'8은 짝수인가요? {is_even(8)}')`를 호출합니다. 괄호 안의 값을 화면 출력과 연결해 읽으세요.
