...
**파이썬 수학 → 함수와 클래스 기초 (11 ~ 20) → class1**에서 가져온 ‘1단계: 가장 간단한 클래스 만들기’ 코드입니다. 단원 전체를 요약하는 대신 이 단계가 담당하는 작업을 한 가지씩 읽습니다. 코드 속 이름·점수·예시 자료는 교재의 연습 데이터이며 실제 학생의 기록이나 성과를 소개하는 자료가 아닙니다.
...
- **1행**: `Sample` 클래스에 상태와 행동을 묶습니다. 이 이름으로 만든 객체의 값은 객체마다 달라질 수 있습니다.
- **4행**: `x`에 `Sample()`의 값을 저장합니다.
- **5행**: `print(type(x))`를 호출합니다. 괄호 안의 값을 화면 출력과 연결해 읽으세요.
