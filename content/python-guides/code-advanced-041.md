...
**파이썬 심화 → tkinter로 Flash Card 만들기 → Flash Card 3**에서 가져온 ‘1단계: CSV 데이터를 카드 리스트로 준비하기’ 코드입니다. 단원 전체를 요약하는 대신 이 단계가 담당하는 작업을 한 가지씩 읽습니다. 코드 속 이름·점수·예시 자료는 교재의 연습 데이터이며 실제 학생의 기록이나 성과를 소개하는 자료가 아닙니다.
...
- **5행**: `BACKGROUND_COLOR`에 `'#B1DDC6'`의 값을 저장합니다.
- **7행**: `data`에 `pandas.read_csv('data/english_words.csv')`의 값을 저장합니다.
- **8행**: `print(data)`를 호출합니다. 괄호 안의 값을 화면 출력과 연결해 읽으세요.
- **10행**: `to_learn`에 `data.to_dict(orient='records')`의 값을 저장합니다.
- **11행**: `print(to_learn)`를 호출합니다. 괄호 안의 값을 화면 출력과 연결해 읽으세요.
- **13행**: `current_card`에 `dict()`의 값을 저장합니다.
