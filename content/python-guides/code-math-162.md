...
**파이썬 수학 → 방정식과 시각화 (51 ~ 60) → 부등식 기초**에서 가져온 ‘세트 1: 리스트 내포로 짝수 제곱 리스트 만들기’ 코드입니다. 단원 전체를 요약하는 대신 이 단계가 담당하는 작업을 한 가지씩 읽습니다. 코드 속 이름·점수·예시 자료는 교재의 연습 데이터이며 실제 학생의 기록이나 성과를 소개하는 자료가 아닙니다.
...
- **1행**: `my_list`에 `[i ** 2 for i in range(10) if i % 2 == 0]`의 값을 저장합니다.
- **2행**: `print(my_list)`를 호출합니다. 괄호 안의 값을 화면 출력과 연결해 읽으세요.
