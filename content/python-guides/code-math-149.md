...
**파이썬 수학 → 클래스와 배열 (41 ~ 50) → list내포**에서 가져온 ‘세트 4: 리스트 내포로 짝수 제곱 리스트 만들기’ 코드입니다. 단원 전체를 요약하는 대신 이 단계가 담당하는 작업을 한 가지씩 읽습니다. 코드 속 이름·점수·예시 자료는 교재의 연습 데이터이며 실제 학생의 기록이나 성과를 소개하는 자료가 아닙니다.
...
print('리스트 내포로 만든 짝수 제곱 리스트:', squared_evens)
```

아래 행 번호는 위 코드 블록의 첫 줄을 1행으로 셉니다. 긴 예제에서는 주요 문장 10개까지 짚었습니다. 함수 안의 줄은 함수를 호출할 때 실행되므로, 행 번호 순서와 실제 실행 순서가 항상 같은 것은 아닙니다.

- **1행**: `squared_evens`에 `[x ** 2 for x in range(10) if x % 2 == 0]`의 값을 저장합니다.
- **3행**: `print('리스트 내포로 만든 짝수 제곱 리스트:', squared_evens)`를 호출합니다. 괄호 안의 값을 화면 출력과 연결해 읽으세요.
...
```text
리스트 내포로 만든 짝수 제곱 리스트: [0, 4, 16, 36, 64]
```
