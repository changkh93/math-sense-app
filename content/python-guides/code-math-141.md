...
**파이썬 수학 → 클래스와 배열 (41 ~ 50) → 클래스로 정수문제**에서 가져온 ‘세트 4: 변수 교환으로 피보나치 수 구하기’ 코드입니다. 단원 전체를 요약하는 대신 이 단계가 담당하는 작업을 한 가지씩 읽습니다. 코드 속 이름·점수·예시 자료는 교재의 연습 데이터이며 실제 학생의 기록이나 성과를 소개하는 자료가 아닙니다.
...
print(f'피보나치 수열 변수 교환 방식 6번째 수는? {fibo_vars(6)}')
```

아래 행 번호는 위 코드 블록의 첫 줄을 1행으로 셉니다. 긴 예제에서는 주요 문장 10개까지 짚었습니다. 함수 안의 줄은 함수를 호출할 때 실행되므로, 행 번호 순서와 실제 실행 순서가 항상 같은 것은 아닙니다.

- **1행**: `fibo_vars(n)` 함수를 정의합니다. 정의와 실제 호출 시점은 구분합니다.
- **2행**: `n <= 0`를 검사합니다. 참인 경로와 그렇지 않은 경로에서 바뀌는 값을 나누어 보세요.
- **3행**: `'n은 1 이상이어야 해요!'`을 호출한 곳으로 돌려주고 이번 함수 실행을 끝냅니다.
- **5행**: `(a, b)`에 `(1, 1)`의 값을 저장합니다.
- **7행**: `n == 1 or n == 2`를 검사합니다. 참인 경로와 그렇지 않은 경로에서 바뀌는 값을 나누어 보세요.
- **8행**: `1`을 호출한 곳으로 돌려주고 이번 함수 실행을 끝냅니다.
- **10행**: `range(n - 2)`에서 값을 하나씩 꺼내 `_`로 읽습니다. 들여쓴 본문이 반복 범위입니다.
- **11행**: `(a, b)`에 `(b, a + b)`의 값을 저장합니다.
- **13행**: `b`을 호출한 곳으로 돌려주고 이번 함수 실행을 끝냅니다.
- **15행**: `print(f'피보나치 수열 변수 교환 방식 6번째 수는? {fibo_vars(6)}')`를 호출합니다. 괄호 안의 값을 화면 출력과 연결해 읽으세요.
...
a,b=b,a+b는 오른쪽을 먼저 계산한 뒤 두 변수에 함께 넣습니다. a=b를 먼저 실행하고 b=a+b를 다음 줄에 쓰면 원래 a가 사라져 다른 계산이 됩니다.

**확인 활동:** 각 반복이 끝난 뒤 (a,b)를 적어 보세요. 이전 b가 새 a가 되는지 확인하세요.
