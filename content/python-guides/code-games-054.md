...
**게임 프로젝트 → 화성 탐사대 I · 고급 Pygame 기초 → 숫자로 만드는 화성 지도**에서 가져온 ‘01-2 메인 반복문 다시 작성’ 코드입니다. 단원 전체를 요약하는 대신 이 단계가 담당하는 작업을 한 가지씩 읽습니다. 코드 속 이름·점수·예시 자료는 교재의 연습 데이터이며 실제 학생의 기록이나 성과를 소개하는 자료가 아닙니다.
...
- **7행**: `screen.fill((15, 25, 42))`를 호출합니다. 호출 앞뒤로 대상의 상태가 달라지는지 살펴보세요.
