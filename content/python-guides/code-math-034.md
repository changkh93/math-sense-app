...
**파이썬 수학 → 기초 자료형과 제어문 (1 ~ 10) → for반복문**에서 가져온 ‘세트 1: for 반복문으로 숫자 제곱하기’ 코드입니다. 단원 전체를 요약하는 대신 이 단계가 담당하는 작업을 한 가지씩 읽습니다. 코드 속 이름·점수·예시 자료는 교재의 연습 데이터이며 실제 학생의 기록이나 성과를 소개하는 자료가 아닙니다.
...
- **1행**: `nums`에 `[1, 7, 3, 4, 5, 10]`의 값을 저장합니다.
- **3행**: `print('--- 각 숫자의 제곱 ---')`를 호출합니다. 괄호 안의 값을 화면 출력과 연결해 읽으세요.
- **5행**: `nums`에서 값을 하나씩 꺼내 `num`로 읽습니다. 들여쓴 본문이 반복 범위입니다.
- **6행**: `print(num ** 2)`를 호출합니다. 괄호 안의 값을 화면 출력과 연결해 읽으세요.
