...
**파이썬 수학 → 클래스와 배열 (41 ~ 50) → numpy array생성**에서 가져온 ‘세트 1: or 조건으로 2의 배수 또는 3의 배수 찾기’ 코드입니다. 단원 전체를 요약하는 대신 이 단계가 담당하는 작업을 한 가지씩 읽습니다. 코드 속 이름·점수·예시 자료는 교재의 연습 데이터이며 실제 학생의 기록이나 성과를 소개하는 자료가 아닙니다.

학습 목표는 코드를 그대로 입력하는 데서 멈추지 않고, 어떤 값이 준비되고 어떤 조건에서 결과가 달라지는지를 설명하는 것입니다. 다음 질문에 먼저 답을 적고 코드와 대조해 보세요.

> 조건 `a > 4`가 참일 때와 거짓일 때, 다음에 실행되는 줄은 무엇인가요?
...
print('a > 4 or b > 5 의 결과는?', a > 4 or b > 5)

c = range(1, 31)

print('2의 배수 또는 3의 배수 for문')
...
- **1행**: `a`에 `3`의 값을 저장합니다.
- **2행**: `b`에 `10`의 값을 저장합니다.
- **4행**: `print('a > 4 or b > 5 의 결과는?', a > 4 or b > 5)`를 호출합니다. 괄호 안의 값을 화면 출력과 연결해 읽으세요.
- **6행**: `c`에 `range(1, 31)`의 값을 저장합니다.
- **8행**: `print('2의 배수 또는 3의 배수 for문')`를 호출합니다. 괄호 안의 값을 화면 출력과 연결해 읽으세요.
- **10행**: `c`에서 값을 하나씩 꺼내 `i`로 읽습니다. 들여쓴 본문이 반복 범위입니다.
- **11행**: `i % 2 == 0 or i % 3 == 0`를 검사합니다. 참인 경로와 그렇지 않은 경로에서 바뀌는 값을 나누어 보세요.
- **12행**: `print(i, end=' ')`를 호출합니다. 괄호 안의 값을 화면 출력과 연결해 읽으세요.
- **14행**: `print()`를 호출합니다. 괄호 안의 값을 화면 출력과 연결해 읽으세요.
- **16행**: `multiples_of_2_or_3`에 `[i for i in c if i % 2 == 0 or i % 3 == 0]`의 값을 저장합니다.
...
이 글의 확인 질문은 **조건 `a > 4`가 참일 때와 거짓일 때, 다음에 실행되는 줄은 무엇인가요?** 입니다. 보호자는 정답을 먼저 알려주기보다 아이가 실제 변수나 조건을 짚어 설명하는지 들어보세요. 어려워하면 단원 전체를 다시 시키기보다 위 핵심 줄 중 막힌 한 줄로 범위를 좁힙니다.
