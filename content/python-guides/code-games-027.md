...
**게임 프로젝트 → 심화 (11 ~ 20) → wanted monster, round_time 표현하기**에서 가져온 ‘1단계: Game 지휘관이 추가된 전체 구조 잡기’ 코드입니다. 단원 전체를 요약하는 대신 이 단계가 담당하는 작업을 한 가지씩 읽습니다. 코드 속 이름·점수·예시 자료는 교재의 연습 데이터이며 실제 학생의 기록이나 성과를 소개하는 자료가 아닙니다.
...
pygame.display.set_caption('수배 몬스터를 찾아라!')
...
- **8행**: `FPS`에 `60`의 값을 저장합니다.
- **9행**: `clock`에 `pygame.time.Clock()`의 값을 저장합니다.
- **11행**: `window_surface`에 `pygame.display.set_mode((WINDOW_WIDTH, WINDOW_HEIGHT))`의 값을 저장합니다.
- **12행**: `pygame.display.set_caption('수배 몬스터를 찾아라!')`를 호출합니다. 호출 앞뒤로 대상의 상태가 달라지는지 살펴보세요.
- **14행**: `Player` 클래스에 상태와 행동을 묶습니다. 이 이름으로 만든 객체의 값은 객체마다 달라질 수 있습니다.
- **17행**: `Monster` 클래스에 상태와 행동을 묶습니다. 이 이름으로 만든 객체의 값은 객체마다 달라질 수 있습니다.
- **20행**: `Game` 클래스에 상태와 행동을 묶습니다. 이 이름으로 만든 객체의 값은 객체마다 달라질 수 있습니다.
