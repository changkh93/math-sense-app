...
**게임 프로젝트 → 기초 (1 ~ 10) → pygame 시작, window 설정**에서 가져온 ‘게임 창 크기와 제목 정하기’ 코드입니다. 단원 전체를 요약하는 대신 이 단계가 담당하는 작업을 한 가지씩 읽습니다. 코드 속 이름·점수·예시 자료는 교재의 연습 데이터이며 실제 학생의 기록이나 성과를 소개하는 자료가 아닙니다.
...
- **3행**: `WINDOW_WIDTH`에 `800`의 값을 저장합니다.
- **4행**: `WINDOW_HEIGHT`에 `600`의 값을 저장합니다.
- **5행**: `window`에 `pygame.display.set_mode((WINDOW_WIDTH, WINDOW_HEIGHT))`의 값을 저장합니다.
- **6행**: `pygame.display.set_caption('나의 첫 게임 무대!')`를 호출합니다. 호출 앞뒤로 대상의 상태가 달라지는지 살펴보세요.
- **7행**: `pygame.quit()`를 호출합니다. 호출 앞뒤로 대상의 상태가 달라지는지 살펴보세요.
