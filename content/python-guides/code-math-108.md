...
**파이썬 수학 → 수학 알고리즘 (31 ~ 40) → 약수, 공약수 구하기**에서 가져온 ‘세트 1: 기본 약수 함수 만들기’ 코드입니다. 단원 전체를 요약하는 대신 이 단계가 담당하는 작업을 한 가지씩 읽습니다. 코드 속 이름·점수·예시 자료는 교재의 연습 데이터이며 실제 학생의 기록이나 성과를 소개하는 자료가 아닙니다.
...
divisors_of_number = divisors1(12)
...
- **1행**: `divisors1(n)` 함수를 정의합니다. 정의와 실제 호출 시점은 구분합니다.
- **2행**: `result`에 `[]`의 값을 저장합니다.
- **4행**: `range(1, n + 1)`에서 값을 하나씩 꺼내 `i`로 읽습니다. 들여쓴 본문이 반복 범위입니다.
- **5행**: `n % i == 0`를 검사합니다. 참인 경로와 그렇지 않은 경로에서 바뀌는 값을 나누어 보세요.
- **6행**: `result.append(i)`를 호출합니다. 호출 앞뒤로 대상의 상태가 달라지는지 살펴보세요.
- **8행**: `result`을 호출한 곳으로 돌려주고 이번 함수 실행을 끝냅니다.
- **10행**: `divisors_of_number`에 `divisors1(12)`의 값을 저장합니다.
- **11행**: `print(divisors_of_number)`를 호출합니다. 괄호 안의 값을 화면 출력과 연결해 읽으세요.
