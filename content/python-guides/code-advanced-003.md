...
**파이썬 심화 → 심화 주제 (1 ~ 10) → 딕셔너리01**에서 가져온 ‘3단계: 새로운 키-값 쌍 추가하기’ 코드입니다. 단원 전체를 요약하는 대신 이 단계가 담당하는 작업을 한 가지씩 읽습니다. 코드 속 이름·점수·예시 자료는 교재의 연습 데이터이며 실제 학생의 기록이나 성과를 소개하는 자료가 아닙니다.
...
- **1행**: `person`에 `{'name': 'John', 'age': 31, 'city': 'New York'}`의 값을 저장합니다.
- **3행**: `person['job']`에 `'Engineer'`의 값을 저장합니다.
- **4행**: `print(person)`를 호출합니다. 괄호 안의 값을 화면 출력과 연결해 읽으세요.
