...
**게임 프로젝트 → 우주 방어대 → 01. 출격 규칙과 게임 스튜디오 준비**에서 가져온 ‘Pygame 라이브러리 가져오기와 초기화’ 코드입니다. 단원 전체를 요약하는 대신 이 단계가 담당하는 작업을 한 가지씩 읽습니다. 코드 속 이름·점수·예시 자료는 교재의 연습 데이터이며 실제 학생의 기록이나 성과를 소개하는 자료가 아닙니다.
...
- **3행**: `pygame.init()`를 호출합니다. 호출 앞뒤로 대상의 상태가 달라지는지 살펴보세요.
