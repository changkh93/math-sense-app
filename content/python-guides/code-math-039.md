...
**파이썬 수학 → 함수와 클래스 기초 (11 ~ 20) → += 연산, for 반복문 활용한 평균 구하기**에서 가져온 ‘세트 1: 리스트의 각 숫자 제곱 출력하기’ 코드입니다. 단원 전체를 요약하는 대신 이 단계가 담당하는 작업을 한 가지씩 읽습니다. 코드 속 이름·점수·예시 자료는 교재의 연습 데이터이며 실제 학생의 기록이나 성과를 소개하는 자료가 아닙니다.
...
- **2행**: `nums`에서 값을 하나씩 꺼내 `num`로 읽습니다. 들여쓴 본문이 반복 범위입니다.
- **3행**: `print(num ** 2)`를 호출합니다. 괄호 안의 값을 화면 출력과 연결해 읽으세요.
