...
**파이썬 심화 → tkinter로 Flash Card 만들기 → Flash Card 2**에서 가져온 ‘1단계: DataFrame을 카드 리스트로 바꾸기’ 코드입니다. 단원 전체를 요약하는 대신 이 단계가 담당하는 작업을 한 가지씩 읽습니다. 코드 속 이름·점수·예시 자료는 교재의 연습 데이터이며 실제 학생의 기록이나 성과를 소개하는 자료가 아닙니다.
...
print(to_learn)
print(type(to_learn))
print(type(to_learn[0]))
```

아래 행 번호는 위 코드 블록의 첫 줄을 1행으로 셉니다. 긴 예제에서는 주요 문장 10개까지 짚었습니다. 함수 안의 줄은 함수를 호출할 때 실행되므로, 행 번호 순서와 실제 실행 순서가 항상 같은 것은 아닙니다.

- **4행**: `data`에 `pd.DataFrame({'English': ['apple', 'banana', 'cherry'], 'Korean': ['사과', '바나나', '체리']})`의 값을 저장합니다.
- **9행**: `print(data)`를 호출합니다. 괄호 안의 값을 화면 출력과 연결해 읽으세요.
- **11행**: `to_learn`에 `data.to_dict(orient='records')`의 값을 저장합니다.
- **13행**: `print(to_learn)`를 호출합니다. 괄호 안의 값을 화면 출력과 연결해 읽으세요.
- **14행**: `print(type(to_learn))`를 호출합니다. 괄호 안의 값을 화면 출력과 연결해 읽으세요.
- **15행**: `print(type(to_learn[0]))`를 호출합니다. 괄호 안의 값을 화면 출력과 연결해 읽으세요.
...
1  banana    바나나
2  cherry     체리
[{'English': 'apple', 'Korean': '사과'}, {'English': 'banana', 'Korean': '바나나'}, {'English': 'cherry', 'Korean': '체리'}]
<class 'list'>
<class 'dict'>
```
