...
**파이썬 수학 → 함수와 클래스 기초 (11 ~ 20) → _args**에서 가져온 ‘세트 2: 두 리스트의 합집합 함수 만들기’ 코드입니다. 단원 전체를 요약하는 대신 이 단계가 담당하는 작업을 한 가지씩 읽습니다. 코드 속 이름·점수·예시 자료는 교재의 연습 데이터이며 실제 학생의 기록이나 성과를 소개하는 자료가 아닙니다.
...
- **9행**: `print(union([1, 2, 3, 4], [3, 4, 5, 6]))`를 호출합니다. 괄호 안의 값을 화면 출력과 연결해 읽으세요.
