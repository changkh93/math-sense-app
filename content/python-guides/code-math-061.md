...
**파이썬 수학 → 함수와 클래스 기초 (11 ~ 20) → 함수**에서 가져온 ‘세트 1: 제곱을 돌려주는 함수 만들기’ 코드입니다. 단원 전체를 요약하는 대신 이 단계가 담당하는 작업을 한 가지씩 읽습니다. 코드 속 이름·점수·예시 자료는 교재의 연습 데이터이며 실제 학생의 기록이나 성과를 소개하는 자료가 아닙니다.
...
- **1행**: `square_num(num)` 함수를 정의합니다. 정의와 실제 호출 시점은 구분합니다.
- **2행**: `num ** 2`을 호출한 곳으로 돌려주고 이번 함수 실행을 끝냅니다.
- **4행**: `a`에 `square_num(5)`의 값을 저장합니다.
- **5행**: `print('5의 제곱에 10을 뺀 값은?', a - 10)`를 호출합니다. 괄호 안의 값을 화면 출력과 연결해 읽으세요.
