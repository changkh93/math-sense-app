...
**게임 프로젝트 → 화성 탐사대 I · 고급 Pygame 기초 → 점프와 화면 연결, 시작점 복귀**에서 가져온 ‘06-1 Explorer.jump 메서드 다시 작성’ 코드입니다. 단원 전체를 요약하는 대신 이 단계가 담당하는 작업을 한 가지씩 읽습니다. 코드 속 이름·점수·예시 자료는 교재의 연습 데이터이며 실제 학생의 기록이나 성과를 소개하는 자료가 아닙니다.
...
- **2행**: `self.velocity.y`에 `-self.jump_speed`의 값을 저장합니다.
- **3행**: `self.grounded`에 `False`의 값을 저장합니다.
