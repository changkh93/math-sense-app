...
test('duplicate finalization pays once, nests rewarded status correctly and wakes subsequent tiers', async () => {
  const db = fixture(40);
  await service.reconcileCrewGrowthEventV2(db, 'a', { nowMs: now });
  ...
  assert.equal(db.data.get('users/u0').crystals, 505);
  const crew = db.data.get('crews/a');
  assert.equal(crew.growthEventV2.tiers.t10.status, 'rewarded');
  assert.equal(crew['growthEventV2.tiers'], undefined);
  assert.equal(crew.growthEventV2.tiers.t20.status, 'verifying');
  await service.reconcileCrewGrowthEventV2(db, 'a', { ...options, nowMs: now + 2 * VERIFICATION_HOLD_MS });
  assert.equal(db.data.get('users/u0').crystals, 1505);
  assert.equal(db.data.get('crews/a').growthEventV2.tiers.t40.status, 'verifying');
  await service.reconcileCrewGrowthEventV2(db, 'a', { ...options, nowMs: now + 3 * VERIFICATION_HOLD_MS });
  assert.equal(db.data.get('users/u0').crystals, 5505);
});
test('replacement participant cannot inherit a departed snapshot participant countdown', async () => {
  const db = fixture(10);
  await service.reconcileCrewGrowthEventV2(db, 'a', { nowMs: now });
  db.data.get('crews/a').memberIds = [...Array.from({ length: 9 }, (_, i) => `u${i}`), 'replacement'];
  ...
  await service.reconcileCrewGrowthEventV2(db, 'a', { nowMs: now + 1000 });
  assert.equal(db.data.get('crews/a').growthEventV2.tiers.t10.status, 'collecting');
});
test('payout rechecks actual membership', async () => {
  const db = fixture(10);
  await service.reconcileCrewGrowthEventV2(db, 'a', { nowMs: now });
  db.data.get('users/u1').crewId = 'b';
  await service.payoutTierReward(db, 'a', 't10', { nowMs: now + VERIFICATION_HOLD_MS, recordCrystalTransactionFn: ledger });
...
test('tier budget holds the whole roster without partially paying', async () => {
  const db = fixture(10);
  await service.reconcileCrewGrowthEventV2(db, 'a', { nowMs: now });
  db.data.set('systemConfig/crewGrowthEvent', { maxTierPayoutOre: 4999 });
  await service.payoutTierReward(db, 'a', 't10', { nowMs: now + VERIFICATION_HOLD_MS, recordCrystalTransactionFn: ledger });
  assert.equal(db.data.get('users/u0').crystals, 5);
  assert.equal(db.data.get('crews/a').growthEventV2.tiers.t10.payoutBlockedReason, 'payout_capacity_or_budget');
});
