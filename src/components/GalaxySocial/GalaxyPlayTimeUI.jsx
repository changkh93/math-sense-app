import { Clock3, Compass, LogOut, RefreshCw, ShieldCheck, Sparkles } from 'lucide-react'
...
export function GalaxyTimeWarning({ stage }) {
  ...
  return <div className={`galaxy-play-time-warning stage-${stage}`} role="status" aria-live="polite">{text}</div>
}
...
export function GalaxyPlayTimeStyles() {
  return <style>{`
    ...
    .galaxy-play-reconnect{position:fixed;z-index:7000;top:max(92px,calc(env(safe-area-inset-top) + 72px));left:50%;transform:translateX(-50%);display:flex;gap:10px;align-items:center;width:min(390px,calc(100vw - 28px));padding:12px 14px;border-radius:15px;color:#dff9ff;background:rgba(8,27,41,.94);border:1px solid rgba(108,211,244,.28);box-shadow:0 15px 45px rgba(0,0,0,.4)}.galaxy-play-reconnect svg{color:#73dcff;flex:0 0 auto}.galaxy-play-reconnect span{display:grid}.galaxy-play-reconnect small{color:#8da9b7;font-size:.7rem;margin-top:2px}
    .galaxy-play-time-warning{position:fixed;z-index:6100;top:max(18px,env(safe-area-inset-top));left:50%;transform:translateX(-50%);width:min(560px,calc(100vw - 170px));padding:12px 16px;border-radius:14px;text-align:center;color:#ffe9b2;background:rgba(46,31,10,.9);border:1px solid rgba(255,204,92,.28);box-shadow:0 12px 34px rgba(0,0,0,.32);font-size:.8rem;font-weight:750;backdrop-filter:blur(14px)}
