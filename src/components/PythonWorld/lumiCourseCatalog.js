...
export const VERTICAL_SLICE_MISSIONS = [
  ...
  },
  {
    ...
    title: 'Field Test: 자유 항로',
    eyebrow: 'ACT 0 · MOVEMENT CORE FIELD TEST',
    objective: '지뢰 지대를 피해 구조 비콘까지 안전하게 이동한 뒤, say()로 구조 완료 신호를 보내세요.',
    briefing: 'LUMI의 기본 기동 시험이 시작되었습니다. 지뢰 지대를 피해 구조 비콘까지 이동하세요. 안전하게 도착한 뒤 say()를 사용해 구조 완료 신호를 전송하세요. 항로는 자유입니다.',
    concepts: ['자유 항로', '명령 조합', '각도 조절', '신호 전송'],
    isFreeRoute: true,
    freeRouteInfo: '🧭 정해진 정답 코드는 없습니다. 자신만의 항로를 만들어 보세요.',
    showSolution: false,
    ...
    pygameBridge: {
      ...
      commonIdea: '자유로운 각도와 이동을 조합하여 목표 지점에 도달하고 신호를 전송한다.',
    },
    memoryFragment: {
      label: '자유 항로 기동 프로토콜',
      code: 'lumi.wake()\n# 자유 항로를 탐색하세요',
    ...
    starterCode: '# LUMI의 기본 기동 시험입니다.\n# 지뢰를 피해 구조 비콘까지 이동하세요.\n# 안전하게 도착한 뒤 신호를 보내세요.\n# 1. wake 명령으로 루미를 깨우세요.\n# 2. 자신만의 회전·이동 항로를 만들어 비콘에 도달하세요.\n# 3. say 명령으로 도착 신호를 전송하세요.\n',
    world: {
      width: 8,
      height: 6,
      capabilities: {
      ...
      rover: { x: 1.0, y: 4.8, direction: 0, energy: 100, awake: false, collisionRadius: 0.2 },
      target: { x: 6.5, y: 1.2, radius: 0.9, kind: 'sos', label: '구조 비콘' },
      obstacles: [
        { id: 'mine_1', x: 3.6, y: 3.2, radius: 0.42, collisionRadius: 0.32 },
        { id: 'mine_2', x: 2.6, y: 3.8, radius: 0.42, collisionRadius: 0.32 },
        { id: 'mine_3', x: 4.2, y: 2.2, radius: 0.42, collisionRadius: 0.32 },
        { id: 'mine_4', x: 1.8, y: 1.2, radius: 0.42, collisionRadius: 0.32 },
        { id: 'mine_5', x: 5.4, y: 0.3, radius: 0.42, collisionRadius: 0.32 },
        { id: 'mine_6', x: 6.8, y: 4.6, radius: 0.42, collisionRadius: 0.32 },
      ],
    },
    goals: [
      { type: 'awake', label: 'LUMI 깨우기' },
      { type: 'noMineCollision', label: '지뢰 충돌 없이 이동' },
      { type: 'reachedTargetArea', label: '구조 비콘 도착' },
      { type: 'sayAfterArrival', label: '도착 후 구조 신호 전송' },
    ],
    conceptEvidence: {
      mustCall: ['lumi.wake', 'lumi.move', 'lumi.say'],
    },
    hints: [
      { level: 1, type: 'context', text: '먼저 LUMI에서 구조 비콘까지 가는 길을 눈으로 찾아보세요. 직선 항로에는 지뢰가 있습니다.' },
      { level: 2, type: 'concept', text: '`lumi.turn(90)`만 사용할 필요는 없습니다. `lumi.turn(45)`, `lumi.turn(-60)`처럼 원하는 각도만큼 회전할 수 있습니다.' },
      { level: 3, type: 'plan', text: '`lumi.wake()` 후 회전(`turn`)과 이동(`move`)을 조합해 지뢰를 피하고, 비콘에 도착한 뒤 `lumi.say(...)`로 신호를 전송하세요.' },
    ],
    hiddenVariants: [],
  },
]
