  ...
  missions: [
    {
      ...
      starterCode: [
        'from metasense import lumi, world',
        '',
...
const IF_MISSION_SET = Object.freeze({
  ...
  missions: [
    createAlphaMission({
      ...
      concepts: ['if', '비교', '상태'],
      starterCode: ['from metasense import lumi', '', 'if lumi.energy < 30:', '    lumi.charge()'],
    ...
    createAlphaMission({
      ...
      concepts: ['if', '비교', '변수'],
      starterCode: ['from metasense import lumi, world', '', 'distance = world.target_distance', '# 조건이 안전할 때 이동하세요.'],
    ...
    createAlphaMission({
      ...
      concepts: ['if', 'for', '속성'],
      starterCode: ['from metasense import lumi', '', 'objects = lumi.scan()', 'for obj in objects:', '    # 강한 신호만 수집하세요.'],
    ...
    createAlphaMission({
      ...
      concepts: ['if', '좌표', '분기'],
      starterCode: ['from metasense import lumi, world', '', 'if world.snapshot()["target"]["x"] < lumi.x:', '    lumi.turn(180)', '', 'lumi.move(world.target_distance)'],
    ...
    createAlphaMission({
      ...
      starterCode: ['from metasense import lumi, world', '', 'distance = world.target_distance', '# and로 두 조건을 결합하세요.'],
    ...
    createAlphaMission({
      ...
      starterCode: ['from metasense import lumi, world', '', '# 1. 필요하면 충전', '# 2. 강한 신호 수집', '# 3. 비콘으로 이동'],
...
const WHILE_MISSION_SET = Object.freeze({
  ...
  missions: [
    createAlphaMission({
      ...
      starterCode: ['from metasense import lumi, world', '', 'while world.target_distance > 0:', '    lumi.move(1)'], goal: { type: 'position', x: 6, y: 2 }, mustUse: ['while'], mustCall: ['lumi.move'],
    ...
    createAlphaMission({
      ...
      starterCode: ['from metasense import lumi, world', '', 'while lumi.energy < 50:', '    lumi.charge()', '', 'lumi.move(world.target_distance)'],
    ...
    createAlphaMission({
      ...
      starterCode: ['from metasense import lumi, world', '', 'while world.objects:', '    obj = world.objects[0]', '    lumi.collect(obj)'],
    ...
    createAlphaMission({
      ...
      starterCode: ['from metasense import lumi, world', '', '# 에너지와 거리를 함께 검사하세요.'], goal: { type: 'position', x: 6, y: 2 }, mustUse: ['while'], mustCall: ['lumi.move'],
    ...
    createAlphaMission({
      ...
      starterCode: ['from metasense import lumi, world', '', '# 신호가 남은 동안 수집', '', '# 비콘에 도착할 때까지 이동'], world: { objects: [{ id: 's1', kind: 'signal', x: 1, y: 2 }, { id: 's2', kind: 'signal', x: 1, y: 2 }] }, goals: [{ type: 'allSignalsCollected' }, { type: 'position', x: 6, y: 2 }], mustUse: ['while'], mustCall: ['lumi.collect', 'lumi.move'], api: [...NAV_API, ...SENSOR_API],
...
const FUNCTION_MISSION_SET = Object.freeze({
  ...
  missions: [
    createAlphaMission({
      ...
      starterCode: ['from metasense import lumi, world', '', 'def move_to_beacon():', '    lumi.move(world.target_distance)', '', 'move_to_beacon()'], goal: { type: 'position', x: 6, y: 2 }, mustUse: ['function'], mustCall: ['lumi.move'],
    ...
    createAlphaMission({
      ...
      starterCode: ['from metasense import lumi, world', '', 'def travel(distance):', '    # 전달받은 거리만큼 이동', '', 'travel(world.target_distance)'], goal: { type: 'position', x: 6, y: 2 }, mustUse: ['function'], mustCall: ['lumi.move'],
    ...
    createAlphaMission({
      ...
      starterCode: ['from metasense import lumi', '', 'def is_safe():', '    # Boolean 값을 반환하세요.', '', 'if is_safe():', '    print("SAFE")'], world: { target: { x: 1, y: 2 } }, goal: { type: 'stdoutIncludes', value: 'SAFE' }, mustUse: ['function', 'return', 'if', 'comparison'],
    ...
    createAlphaMission({
      ...
      starterCode: ['from metasense import lumi', '', 'def rescue(signal):', '    # 우선순위가 높은 신호만 수집', '', 'for signal in lumi.scan():', '    rescue(signal)'], world: { target: { x: 1, y: 2 }, objects: [{ id: 'low', kind: 'signal', x: 1, y: 2, priority: 1 }, { id: 'high', kind: 'signal', x: 1, y: 2, priority: 4 }] }, goal: { type: 'collectedIncludes', id: 'high' }, mustUse: ['function', 'if', 'for', 'comparison'], mustCall: ['lumi.scan', 'lumi.collect'], api: SENSOR_API,
    ...
    createAlphaMission({
      ...
      starterCode: ['from metasense import lumi, world', '', 'def prepare():', '    pass', '', 'def rescue():', '    pass', '', 'def navigate():', '    pass', '', 'prepare()', 'rescue()', 'navigate()'], world: { rover: { x: 1, y: 2, direction: 0, energy: 2, maxEnergy: 100 }, objects: [{ id: 'station', kind: 'charge', x: 1, y: 2 }, { id: 'rescue', kind: 'signal', x: 1, y: 2, priority: 5 }] }, goals: [{ type: 'collectedCount', count: 1 }, { type: 'position', x: 6, y: 2 }], mustUse: ['function', 'if', 'for', 'comparison'], mustCall: ['lumi.charge', 'lumi.scan', 'lumi.collect', 'lumi.move'], api: [...NAV_API, ...SENSOR_API],
