...
function automationMission({
  ...
  return {
    ...
    memoryFragment: {
      label: '손상된 자동화 신호',
      code: '# for 이름 in range(횟수):\n#     반복할 명령\n# 들여쓰기와 반복 범위를 직접 완성하세요.',
      duration: 1800,
      autoPlay: true,
    },
    scaffold: {
...
export const LUMI_AUTOMATION_CORE_MISSIONS = [
  automationMission({
    id: 'lumi-automation-5-01', codeName: '5-1', order: 1,
    title: '세 번의 추진 신호',
    ...
    starterCode: 'for step in range(1):\n    lumi.move(1)\n\n# TODO: 반복 횟수를 고쳐 비콘에 도달하세요.',
    goals: [{ type: 'position', x: 4, y: 2, label: '한 칸 이동을 세 번 반복해 도달' }],
    mustUse: ['for', 'range'],
    mustCall: ['lumi.move'],
    hints: ['`range(3)`은 0, 1, 2를 차례로 만들어 세 번 반복합니다.'],
    hiddenVariants: [
      { id: 'shifted-start', world: { rover: { x: 2, y: 3 }, target: { x: 5, y: 3 } }, goals: [{ type: 'position', x: 5, y: 3, label: '옮겨진 항로에서도 세 번 이동' }] },
    ],
  }),
  automationMission({
    id: 'lumi-automation-5-02', codeName: '5-2', order: 2,
    title: '가변 항로 자동화',
    ...
    starterCode: 'distance = world.steps_to_target\n\n# TODO: distance 횟수만큼 한 칸 이동하세요.\nfor step in range(1):\n    lumi.move(1)',
    ...
    hints: ['숫자 1 대신 이미 저장된 `distance`를 `range()`에 전달하세요.'],
    hiddenVariants: [
      { id: 'near-route', target: { x: 4, y: 2 }, goals: [{ type: 'position', x: 4, y: 2, label: '짧아진 항로에 적응' }] },
      { id: 'far-route', target: { x: 9, y: 2 }, goals: [{ type: 'position', x: 9, y: 2, label: '길어진 항로에 적응' }] },
    ],
  }),
  automationMission({
    id: 'lumi-automation-5-03', codeName: '5-3', order: 3,
    title: '반복 번호 관찰',
    objective: '조사할 행의 수만큼 반복하고 마지막 반복 번호를 보고하세요.',
    briefing: 'for의 step은 장식이 아닙니다. 반복할 때마다 0부터 하나씩 달라지는 값을 Memory Core에서 관찰하세요.',
    concepts: ['반복 변수', 'range', '순차 값', '센서'],
    starterCode: 'row_count = world.survey_rows\n\nfor step in range(1):\n    lumi.say(step)\n\n# TODO: 모든 행 번호를 차례로 보고하세요.',
    ...
    hints: ['`range(row_count)`를 사용하면 행 수가 바뀌어도 마지막 번호가 자동으로 달라집니다.'],
    hiddenVariants: [
      {
        id: 'five-rows', world: { surveyRows: 5 },
        goals: [
  ...
  automationMission({
    id: 'lumi-automation-5-04', codeName: '5-4', order: 4,
    title: '에너지 누적기',
    objective: '1부터 신호 개수까지의 에너지를 for로 total에 누적하고 합계를 출력하세요.',
    briefing: '반복문은 행동뿐 아니라 계산도 자동화합니다. 0에서 시작한 누적 변수에 반복 번호를 차례로 더하는 패턴을 익히세요.',
    concepts: ['for', 'range', '누적 변수', '총합'],
    starterCode: 'signal_count = world.survey_columns\ntotal = 0\n\n# TODO: 1부터 signal_count까지 total에 누적하세요.\n\nprint(total)',
    ...
    hints: ['`range(1, signal_count + 1)`은 1부터 마지막 신호 번호까지 만듭니다.', '반복문 안에서 `total = total + energy`로 갱신하세요.'],
    hiddenVariants: [
      {
        id: 'six-signals', world: { surveyColumns: 6 },
        goals: [
  ...
  automationMission({
    id: 'lumi-automation-5-05', codeName: '5-5', order: 5,
    title: '신호 전량 회수',
    objective: 'scan()이 돌려준 모든 신호를 for로 순회하며 하나도 빠짐없이 수집하세요.',
    briefing: '신호 수를 미리 알 수 없습니다. 스캔 목록 자체를 반복하면 두 개든 네 개든 같은 코드로 처리할 수 있습니다.',
    concepts: ['for', '순회', '리스트', '객체'],
    starterCode: 'signals = lumi.scan()\n\n# TODO: signals 안의 각 신호를 차례로 수집하세요.',
    world: {
      target: { x: 1, y: 2 },
      objects: [
    ...
    hints: ['`for signal in signals:`로 목록을 순회하고, 안에서 `lumi.collect(signal)`을 호출하세요.'],
    hiddenVariants: [
      {
        id: 'extra-signals',
        world: {
  ...
  automationMission({
    id: 'lumi-automation-5-06', codeName: '5-6', order: 6,
    title: '중첩 반복 사각 항로',
    objective: '바깥 반복은 네 변을, 안쪽 반복은 각 변의 칸을 담당하게 해 사각 항로를 완주하세요.',
    briefing: '큰 패턴 안에 작은 패턴이 반복됩니다. 안쪽 for로 한 변을 이동하고, 바깥 for가 그 과정을 네 번 되풀이합니다.',
    concepts: ['중첩 for', '안쪽 반복', '바깥 반복', '회전'],
    starterCode: 'side_length = world.steps_to_target\n\n# TODO: 네 변을 반복하는 바깥 for를 만드세요.\n# TODO: 각 변에서 side_length번 한 칸씩 이동하세요.\n# TODO: 한 변이 끝날 때 90도 회전하세요.',
    ...
    hints: ['바깥쪽은 `range(4)`, 안쪽은 `range(side_length)`입니다.', '안쪽 반복이 끝난 뒤, 바깥 반복 안에서 `lumi.turn(90)`을 실행하세요.'],
    hiddenVariants: [
      {
        id: 'larger-square',
        ...
        goals: [
          ...
          { type: 'eventOccurred', eventType: 'rover_turned', label: '큰 사각 항로 회전' },
        ],
      },
    ],
  }),
  automationMission({
    id: 'lumi-automation-5-07', codeName: '5-F', order: 7,
    title: '격자 조사 자동화',
    objective: '행과 열 센서값으로 중첩 반복을 구성하고 전체 조사 칸 수를 계산해 보고하세요.',
    briefing: 'FINAL AUTOMATION TEST입니다. 조사 구역 크기가 바뀌어도 하드코딩 없이 모든 셀을 세는 자동화 코어를 완성하세요.',
    concepts: ['중첩 for', '센서', '누적 변수', '일반화'],
    starterCode: 'rows = world.survey_rows\ncolumns = world.survey_columns\ncells = 0\n\n# TODO: 행과 열을 중첩 반복하며 cells를 1씩 늘리세요.\n\nlumi.say(cells)',
    ...
    hints: ['바깥 for는 `range(rows)`, 안쪽 for는 `range(columns)`를 사용합니다.', '가장 안쪽에서 `cells = cells + 1`로 한 칸씩 누적하세요.'],
    hiddenVariants: [
      {
        id: 'wide-grid', world: { surveyRows: 3, surveyColumns: 4 },
        goals: [
      ...
      {
        id: 'single-row-grid', world: { surveyRows: 1, surveyColumns: 5 },
        goals: [
          { type: 'variableValueEquals', name: 'cells', value: 5, label: '1 × 5 구역의 5칸 계산' },
          { type: 'spokenMessage', includes: '5', label: '단일 행 구역 보고' },
        ],
      },
    ],
  }),
