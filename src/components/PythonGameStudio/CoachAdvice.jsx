...
export default function CoachAdvice({ uid, source, error, mode = 'file', stale, consent = false, samplesConsent = false, onRequested, onReceived, requestAdvice = requestCoachAdvice }) {
  ...
  const ask = async () => {
    if (lock.current || stale || !payload || !preview) return
  ...
  return <>
    ...
    {preview && !answer && <div className="pgs-coach-preview">
      ...
      <div className="pgs-coach-actions"><button disabled={pending || lock.current} onClick={ask}>{pending ? '힌트를 생각하고 있어요…' : 'AI 힌트 받기'}</button>{!pending && !lock.current && <button onClick={() => setPreview(false)}>돌아가기</button>}</div>
    </div>}
