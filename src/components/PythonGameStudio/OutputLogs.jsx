...
export default function OutputLogs({ logs = [], collapseError = log => Boolean(log.coach), onJumpToError }) {
  const groups = []
  ...
  return groups.map(group => {
    ...
    return collapseError(log) ? <details className="pgs-error-details" key={group.key}><summary>자세한 오류 보기</summary>{content}</details> : <div key={group.key}>{content}</div>
  })
}
