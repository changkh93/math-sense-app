...
import BehaviorCoach from './BehaviorCoach'
...
function AssetPreview({ file, onInsert, onConvert, busy }) {
  ...
  return <div className="pgs-asset-preview">
    ...
    {file.kind === 'font' && <div className="pgs-font-sample" style={{ fontFamily: fontFamily || 'sans-serif' }}>나만의 코드<br />Hello, Code!<br />0123456789</div>}
    {file.kind === 'audio' && !file.path.toLowerCase().endsWith('.ogg') && <><small>코드 실행 시 자동으로 OGG로 변환합니다. 원본은 보존됩니다.</small><button disabled={busy} onClick={onConvert}>OGG 사본 만들기</button></>}
...
export default function PythonGameStudio({ uid = 'local-preview', onBack, publicAccess = false }) {
  ...
  useEffect(() => {
    ...
    if (!output) return
    const latestError = logs.at(-1)?.type === 'ERROR' ? output.querySelector('.pgs-error-details:last-child') : null
    // Keep the traceback in view instead of jumping past it to the hints below.
  ...
  const submitDialog = event => {
    ...
    try {
      if (dialog === 'new') switchProject(createProject(dialogValue.trim() || '나의 프로젝트'))
      if (dialog === 'folder') {
  ...
  return <main className="pgs-shell" data-editor-mode={editorMode} data-code-theme={preferences.theme} onDragOver={e => { if (Array.from(e.dataTransfer.types).includes('Files')) e.preventDefault() }} onDrop={e => { if (!e.dataTransfer.files.length) return; e.preventDefault(); e.stopPropagation(); if (!busy) upload(e.dataTransfer.files) }}>
    <header className="pgs-header">{onBack && <button className="pgs-icon-btn" aria-label={publicAccess ? '메타센스 홈으로' : '수업으로 돌아가기'} onClick={async () => { try { await saveChain.current.catch(() => {}); await saveDraft(uid, project); stop(); onBack() } catch (error) { setNotice(error.message) } }}><ArrowLeft size={20} /></button>}<div className="pgs-brand"><span>METASENSE / {publicAccess ? 'OPEN CODE STUDIO' : 'CODE STUDIO'}</span><input aria-label="프로젝트 이름" value={project.title} maxLength={80} onChange={e => setProject({ ...project, title: e.target.value })} /></div><span className="pgs-save-state" role="status">{saved}</span>{publicAccess && <nav className="pgs-public-links" aria-label="메타센스 안내"><a href="/python/guides/">학습 노트</a><a href="/python">파이썬 과정</a><a href="/#login">로그인</a></nav>}<div className="pgs-header-actions"><button onClick={showLibrary}><FolderOpen size={16} /> 내 프로젝트</button><button aria-label="프로젝트 다운로드" title="프로젝트 다운로드" onClick={() => download(`${project.title || 'game'}.mspygame.json`, JSON.stringify(project), 'application/json')}><Download size={17} /></button></div></header>
    ...
    {drawer && <div className="pgs-modal-backdrop"><section className="pgs-library" role="dialog" aria-modal="true" aria-label="내 프로젝트" aria-busy={importing}><div className="pgs-panel-title">내 프로젝트<button aria-label="프로젝트 목록 닫기" disabled={importing} onClick={() => setDrawer(false)}><X size={18} /></button></div>{notice && <p role="alert">{notice}</p>}<div className="pgs-library-actions"><button disabled={busy} onClick={() => { setDialogValue('나의 프로젝트'); setDialog('new') }}><Plus size={16} /> 새 프로젝트</button><button disabled={busy} onClick={() => folderInput.current.click()}><FolderOpen size={16} /> {importing ? '가져오는 중…' : '프로젝트 가져오기'}</button>{!publicAccess && <><button disabled={busy} onClick={() => importInput.current.click()}><Upload size={16} /> 백업 파일 복원</button><button disabled={busy} onClick={async () => {
      setBusy(true)
      try {
        const { createCoinCollectProject } = await import('./coinCollectTemplate')
        await switchProject(await createCoinCollectProject(), { persist: true })
      } catch (error) { setNotice(error.message) }
      finally { if (mounted.current) setBusy(false) }
    }}>동전 모으기 수업 준비</button><button disabled={busy} onClick={async () => { try { const { createMonsterProject } = await import('./monsterTemplate'); switchProject(await createMonsterProject()) } catch (error) { setNotice(error.message) } }}>몬스터 잡기 수업 준비</button><button disabled={busy} onClick={async () => {
      setBusy(true)
      try {
    ...
    }}>화성 탐사대 수업 준비</button><button disabled={busy} onClick={async () => {
      setBusy(true)
      try {
        const { createFlashCardProject } = await import('./flashCardTemplate')
        await switchProject(await createFlashCardProject(), { persist: true })
      } catch (error) { setNotice(error.message) }
      finally { if (mounted.current) setBusy(false) }
    }}>Flash Card 수업 준비</button></>}</div><p className="pgs-import-help">프로젝트 가져오기: 로컬 폴더의 Python·노트북·이미지·폰트·사운드를 하위 경로와 함께 가져옵니다.{!publicAccess && ' 백업 파일 복원: .mspygame.json 또는 .py/.ipynb 파일을 엽니다.'}</p><h3>이 기기에 저장한 초안</h3>{drafts.map(row => <div className="pgs-project-row" key={row.key}><button className="pgs-project-open" disabled={busy} onClick={() => switchProject(selectDraft(row))}><FileCode2 size={20} /><span>{row.project.title}<small>{new Date(row.savedAt).toLocaleString('ko-KR')}</small></span></button><button className="pgs-project-delete" disabled={busy} aria-label={`${row.project.title} 프로젝트 삭제`} title="프로젝트 삭제" onClick={() => { setNotice(''); setDeleteTarget(row) }}><Trash2 size={17} /></button></div>)}<input ref={folderInput} type="file" webkitdirectory="" directory="" multiple hidden onChange={importFolder} /><input ref={importInput} type="file" accept=".json,.mspygame.json,.py,.ipynb" hidden onChange={importProject} /></section></div>}
