...
export const MAX_TRACE_EVENTS = 1_500

export function evaluatorError(code, message) {
...
export const FORBIDDEN_SOURCE = /(?:\b(?:os|sys|subprocess|shutil|socket|http|urllib|requests|pickle|shelve|eval|exec|compile|open|file|input|globals|locals|getattr|setattr|delattr|breakpoint|globalThis|window|document|process)\b|__)/
...
export class SafePythonInterpreter {
  ...
  execute() {
    ...
    this.emitTrace('function-enter', {
      statementText: `def ${this.entryFunction}(${argEntries.map(([k, v]) => `${k}=${JSON.stringify(v)}`).join(', ')}):`,
      sourceLineHint: 1,
      stateDiff: argEntries.map(([name, val]) => ({
        kind: 'write',
        path: name,
        before: null,
        after: this.snapshotValue(val),
      })),
      metadata: { args: this.args },
    })
...
export function projectSemanticTraceToMeaningful(rawEvents = [], maxScenes = 30) {
...
export function runRestrictedPythonFunction(pythonCode, entryFunction, args = {}, options = {}) {
