...
function buildFeedbackPolicyGuidance(context) {
  ...
  return {
    ...
    rules: [
      ...
      'NEW · 체험 학습 완료 기록은 초등수학의 실제 연습 근거로 인정한다. 카드 정답, 세로셈 미션 완료, 같은 크기 분수·통분·약분 탐구의 최종 완료만 근거로 쓰며 단순 화면 진입, 렌즈 이동, 중간 단계는 완료로 해석하지 않는다.',
      '분수 체험 학습은 같은 크기 분수의 분자·분모를 모두 맞힌 때, 통분한 두 분수와 크기 비교를 모두 맞힌 때, 또는 여러 약분 단계를 거쳐 기약분수에 도달한 때 완료로 해석한다.',
