...
const nav = `<a class="skip" href="#main">본문으로 이동</a><header class="masthead"><a href="/">메타센스</a><nav aria-label="주요 메뉴"><a href="${root}">파이썬 학습 노트</a><a href="/python">과정 안내</a><a href="/python#apply">7일 무료체험</a></nav></header>`;
const footer = `<footer class="footer">둘시네가 운영하는 메타센스 · 스스로 배우는 힘<br><a href="/python">파이썬 과정</a> · <a href="https://blog.naver.com/metasense_edu">공식 블로그</a> · <a href="https://www.instagram.com/metasense_edu/">인스타그램</a> · <a href="https://youtube.com/@metasense_edu">유튜브</a> · <a href="https://pf.kakao.com/_xfxkGDn">카카오 상담</a><br><a href="${root}editorial/">제작 기준과 이미지 출처</a> · <a href="/privacy">개인정보 처리방침</a></footer>`;
const cta = `<section class="cta"><h2>우리 아이에게 맞는 시작점을 찾아보세요.</h2><p>초3부터 · 자율 학습 · 정규 월15만 원 · 기본 7일 무료체험<br>신청 후 1일 이내 연락 · 자동 결제 없음</p><a class="button" href="/python#apply">파이썬 과정·체험 안내</a></section>`;
...
const listCard = a => `<article class="card"><a href="${url(a)}"><img src="${a.image}" alt="${esc(a.imageAlt)}" width="768" height="432" loading="lazy"></a><div><span class="eyebrow">${esc(a.category)}</span><h2><a href="${url(a)}">${esc(a.title)}</a></h2><p>${esc(a.description)}</p></div></article>`;
...
for (const a of articles) {
  ...
  for (const key of ['title','description','answer','imageAlt','imageCaption','published','updated','author']) if(!a[key]) throw new Error(`Missing ${a.slug}/${key}`);
  for(const r of a.related) if(!articles.some(x=>x.slug===r)) throw new Error(`Broken related ${r}`);
  await access(`public${a.image}`);
...
for(let page=1;page<=pages;page++) {
 ...
 await emit(path,document(`파이썬 학습 노트${page>1?` · ${page}페이지`:''}`,'아이의 첫 코드부터 과제와 피드백까지. 메타센스의 실제 도구 화면과 짧은 실험으로 살펴보는 파이썬 학습 안내.',path,`<main id="main" class="wrap"><span class="eyebrow">METASENSE / PYTHON NOTES</span><h1>아이의 코드 한 줄에서<br>시작하는 이야기</h1><p class="lead">언제 시작할지, 막히면 어떻게 도울지.<br>부모님의 질문을 실제 코드와 배움의 과정으로 풀어봅니다.</p><p><a href="${root}editorial/">글과 이미지의 제작 기준</a></p><div class="grid">${articles.slice((page-1)*20,page*20).map(listCard).join('')}</div><nav class="pagination" aria-label="목록 페이지">${Array.from({length:pages},(_,i)=>`<a ${i+1===page?'aria-current="page"':''} href="${i===0?root:`${root}page/${i+1}/`}">${i+1}</a>`).join('')}</nav>${cta}</main>`,[{'@type':'CollectionPage',name:'파이썬 학습 노트',url:base+path}]));
}
const editorial=`${root}editorial/`;
await emit(editorial,document('글과 이미지의 제작 기준','메타센스 파이썬 학습 노트의 자료 출처, 편집 방식, 이미지 구분과 정정 문의 안내.',editorial,`<main id="main" class="wrap article-body"><h1>글과 이미지의 제작 기준</h1><p>이 학습 노트는 메타센스의 공개 과정 안내, 직접 실행한 코드, 공식 기술 문서를 바탕으로 제작합니다. 초안 정리와 삽화 제작에 AI 도구를 활용합니다. 코드의 실행 결과와 공식 자료를 확인하고, 운영 조건은 메타센스가 제공한 안내를 따릅니다.</p><h2>실제 화면을 정확히 설명합니다</h2><p>코드 스튜디오 화면은 실제 도구에서 코드를 실행한 캡처입니다. 특정 학생의 과제나 성과라는 설명을 붙이지 않습니다. 삽화에는 AI 제작 삽화라는 설명을 붙입니다. 학생의 말을 지어내거나 재구성한 기록을 실제 기록으로 소개하지 않습니다.</p><h2>학생 자료와 익명처리</h2><p>개별 학생의 이름·닉네임을 가리는 것만으로 공개 가능하다고 판단하지 않습니다. 학생 자료를 다룰 때는 식별 단서와 공개 이용 권한을 함께 확인합니다. 현재 학습 노트에는 개별 학생의 비공개 평가나 활동 기록을 게시하지 않았습니다.</p><h2>수정과 문의</h2><p>각 글의 수정일은 본문을 실질적으로 바꾼 날에 갱신합니다. 오류나 운영 조건의 변경을 발견하면 <a href="https://pf.kakao.com/_xfxkGDn">카카오 상담</a>에 글 주소와 해당 내용을 알려주세요.</p><p><a href="${root}">학습 노트 목록으로</a></p></main>`));
const paths=['/','/trial','/python',...listingPaths,editorial,...articles.map(url)];
...
await writeFile('dist/llms.txt',`# 메타센스\n\n> 둘시네가 운영하는 수학·파이썬·고전읽기 온라인 교육. 공개 학습 안내 링크 목록입니다.\n\n## 과정\n- [파이썬](https://msense.me/python): 초3부터, 자율 학습, 월15만 원, 기본 7일 무료체험, 신청 후 1일 이내 연락.\n\n## 학습 노트\n${articles.map(a=>`- [${a.title}](${base}${url(a)}): ${a.description}`).join('\n')}\n\n이 목록은 보조 안내이며 크롤링이나 AI 인용을 보장하지 않습니다.\n`);
await writeFile('dist/python-guides/build-manifest.json',JSON.stringify({articles:articles.map(a=>({slug:a.slug,path:url(a),title:a.title})),listingPaths,categories},null,2));
