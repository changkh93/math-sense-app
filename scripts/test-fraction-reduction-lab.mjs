...
assert.match(lab, /frl-quotient-row is-numerator/, 'the new numerator input should occupy the top fraction row')
assert.match(lab, /frl-quotient-row is-denominator/, 'the new denominator input should occupy the bottom fraction row')
assert.match(lab, /frl-quotient-line/, 'the numerator and denominator inputs should be separated by a fraction bar')
assert.match(lab, /\{factorValid && \(/, 'new-fraction inputs should stay hidden until a valid common factor is selected')
assert.match(lab, /destinationRevealed \? finalFraction\.numerator : '\?'/, 'the target numerator should stay hidden before a valid choice')
assert.match(lab, /destinationRevealed \? finalFraction\.denominator : '\?'/, 'the target denominator should stay hidden before a valid choice')
...
assert.match(css, /\.frl-current-equation > \.frl-fraction/, 'the current fraction should have a dedicated enlarged treatment')
assert.match(css, /\.frl-quotient-line\s*\{[^}]*background:\s*#e8fffb/s, 'the input fraction bar should be a short solid line without a gradient')
assert.doesNotMatch(css, /\.frl-history small\s*\{[^}]*display:\s*none/s, 'the hidden target fraction should remain visible on narrow screens')
