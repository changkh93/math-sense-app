...
STEPS = []
TITLES = ['출격 규칙과 코드 스튜디오 준비','화면과 게임 루프, 다섯 설계도','탐사선 표시와 좌우 이동','아군 탄환과 두 발 제한','적의 설계도와 무작위 발사 조건','임시 편대와 적 탄환','Mission과 한글 HUD, 정식 편대','편대 반전과 방어선 침범','전장 정돈과 일시정지','게임오버, 충돌, 다음 라운드']
...
add('09-C','반투명 안내창에 이유 표시하기','기다리는 이유가 보이지 않으면 사용자는 멈춘 줄 알 수 있습니다. 정지된 전장 위에 안내를 그립니다.',
    ...
    '정지 화면에서도 draw와 이벤트 처리는 계속됩니다. 코드 스튜디오 정지 버튼으로 실행을 끝낼 수 있습니다.',
...
파이썬 → **코드 스튜디오** → **내 프로젝트** → **우주 방어대 수업 준비**를 누릅니다. 빈 main.py, assets의 이미지2개·OGG 소리6개·DoHyeon-Regular.ttf, asset_credits.py가 있는지 확인합니다. 수업 준비는 새 프로젝트를 만들므로 매 단원 다시 누르지 않습니다. 같은 프로젝트의 main.py를 계속 고칩니다.
