...
const ids={1:'1788901151685',2:'1788901191909',3:'1788901201841',4:'1788901261901'};
...
for(const round of [1,2,3,4]){
  ...
  assert.equal(questions.length,20);assert.equal(manifest.questions.length,20);
  assert.equal(manifest.round,round);
  assert.equal(manifest.unitId,`reg_1781420075936_chap_1781420235224_unit_${ids[round]}`);
  for(const [i,q]of questions.entries()){
    ...
    assert(typeof m.verification==='string'&&m.verification.trim(),label+' verification');
...
console.log(JSON.stringify({questions:80,mathExpressions:mathCount,imageCount,structuralValidation:'PASS',educationalReview:'still required by Codex'}));
