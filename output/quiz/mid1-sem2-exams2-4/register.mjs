...
const round=Number(process.argv[2]);assert([2,3,4].includes(round));
const out=path.join(base,'authoring',`round${round}`);
const root=path.resolve(base,'../../..');
const UNIT='reg_1781420075936_chap_1781420191007_unit_'+({2:'1788876955807',3:'1788876965426',4:'1788876975928'}[round]);
const PROJECT='math-sense-1f6a8',BUCKET=PROJECT+'.firebasestorage.app';
const IMPORT=`mid1-sem2-exam${round}-20260909-authorized`;
...
assert.equal(unit.data().title.replace(/\s/g,''),`1학년2학기중간${round}회`);
...
if(mode==='inspect'){
  ...
  if(mode==='apply'){
    ...
    await db.runTransaction(async tx=>{
      ...
      assert.equal(freshUnit.data().title.replace(/\s/g,''),`1학년2학기중간${round}회`);
