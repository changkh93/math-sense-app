...
const round=Number(process.argv[2]);assert([5,6].includes(round));
const out=path.join(base,'authoring',`round${round}`);
const root=path.resolve(base,'../../..');
const UNIT='reg_1781420075936_chap_1781420292103_unit_'+({5:'1789033557224',6:'1789033567891'}[round]);
const PROJECT='math-sense-1f6a8',BUCKET=PROJECT+'.firebasestorage.app';
const IMPORT=`mid3-sem2-exam${round}-20260910-authorized`;
...
assert.equal(meta.unitId,UNIT);assert.equal(questions.length,24);
...
const ids=questions.map((_,i)=>`${UNIT}_q${String(i+1).padStart(2,'0')}_20260910`);
const [unit,existing]=await Promise.all([unitRef.get(),query.get()]);
assert(unit.exists,'Target unit missing');
assert.equal(unit.data().title.replace(/\s/g,''),`3학년2학기중간${round}회(삼각비,원,통계)`);
...
if(mode==='inspect'){
  ...
  const payloads=questions.map((q,i)=>{
    ...
    return {id:ids[i],docId:ids[i],unitId:UNIT,question:q.question,
      ...
      source:{importId:IMPORT,sourceQuestionNumber:meta.questions[i].sourceNumber,localQuestionNumber:i+1,sourceTitle:meta.sourceTitle,
        replacedWithNewQuestion:meta.questions[i].replaced,replacementReason:meta.questions[i].replacementReason,approvedRevision:'2026-09-10',contentSHA256:hash(Buffer.from(JSON.stringify(q)))}};
  });
  ...
  if(mode==='apply'){
    ...
    await db.runTransaction(async tx=>{
      ...
      assert.equal(freshUnit.data().title.replace(/\s/g,''),`3학년2학기중간${round}회(삼각비,원,통계)`);
      ...
      tx.update(unitRef,{quizCount:questions.length,lastUpdated:admin.firestore.FieldValue.serverTimestamp(),updatedAt:admin.firestore.FieldValue.serverTimestamp()});
    });
    console.log(`Committed ${questions.length} quiz documents and target unit count/timestamp.`);
  }
  const [after,afterUnit]=await Promise.all([query.get(),unitRef.get()]);
  assert.equal(after.size,questions.length);
  assert.equal(afterUnit.data().quizCount,questions.length);
