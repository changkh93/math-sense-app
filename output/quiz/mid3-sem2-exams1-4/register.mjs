...
const UNIT='reg_1781420075936_chap_1781420292103_unit_'+({1:'1788903283590',2:'1788903293848',3:'1788907111960',4:'1788907123074'}[round]);
const PROJECT='math-sense-1f6a8',BUCKET=PROJECT+'.firebasestorage.app';
const IMPORT=`mid3-sem2-exam${round}-20260909-authorized`;
...
assert.equal(meta.unitId,UNIT);assert.equal(questions.length,({1:25,2:30,3:26,4:19})[round]);
...
assert.equal(unit.data().title.replace(/\s/g,''),`3학년2학기중간${round}회`);
...
if(mode==='inspect'){
  ...
  if(mode==='apply'){
    ...
    await db.runTransaction(async tx=>{
      ...
      assert.equal(freshUnit.data().title.replace(/\s/g,''),`3학년2학기중간${round}회`);
