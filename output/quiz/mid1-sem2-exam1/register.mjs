...
const UNIT='reg_1781420075936_chap_1781420191007_unit_1788876936054';
const PROJECT='math-sense-1f6a8',BUCKET=PROJECT+'.firebasestorage.app';
const IMPORT='mid1-sem2-exam1-20260909-approved';
...
assert.equal(meta.unitId,UNIT);assert.equal(questions.length,25);
...
assert(questions[15].question.includes('같은 도형'));
assert.equal(questions[21].answer,'$30$');
...
assert.equal(unit.data().title.replace(/\s/g,''),'1학년2학기중간1회');
...
if(mode==='inspect'){
  ...
  assert.equal(imagePlan.length,16);
  ...
  const payloads=questions.map((q,i)=>{
    ...
    return {id:ids[i],docId:ids[i],unitId:UNIT,question:q.question,
      ...
      source:{importId:IMPORT,sourceQuestionNumber:i+1,sourceTitle:meta.sourceTitle,
        replacedWithNewQuestion:i===21,approvedRevision:'2026-09-09',contentSHA256:hash(Buffer.from(JSON.stringify(q)))}};
  });
  ...
  if(mode==='apply'){
    ...
    await db.runTransaction(async tx=>{
      ...
      assert.equal(freshUnit.data().title.replace(/\s/g,''),'1학년2학기중간1회');
      ...
      tx.update(unitRef,{quizCount:25,lastUpdated:admin.firestore.FieldValue.serverTimestamp(),updatedAt:admin.firestore.FieldValue.serverTimestamp()});
    });
    console.log('Committed 25 quiz documents and target unit count/timestamp.');
  }
  const [after,afterUnit]=await Promise.all([query.get(),unitRef.get()]);
  assert.equal(after.size,25);
  assert.equal(afterUnit.data().quizCount,25);
  ...
  const report={status:'REGISTERED_AND_VERIFIED',project:PROJECT,unitId:UNIT,title:afterUnit.data().title,
    ...
    newQ22Answer:docs.get(ids[21]).answer,quizIds:ids,verifiedAt:new Date().toISOString(),questionsSHA256:hash(bytes)};
