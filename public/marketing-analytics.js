...
(() => {
  ...
  const publicPath = p => ['/', '/python', '/trial'].includes(p) || /^\/python\/guides\/(?:[a-z0-9-]+\/)*$/.test(p);
  const path = () => location.pathname.replace(/\/$/, '') || '/';
  const allowed = () => publicPath(path()) || /^\/python\/guides\/(?:[a-z0-9-]+\/)*$/.test(location.pathname);
  if (!allowed()) return;
  ...
  const sources = new Set(['naver','instagram','youtube','kakao','clip']);
  ...
  if (query.get('utm_campaign') === 'python_trial') campaign.campaign_name = 'python_trial';
  ...
  function forward(item) {
    ...
    gtag('event', item.event, {...params(),funnel:'python',label:labels.has(item.label)?item.label:''});
  }
  ...
  document.addEventListener('click',e=>{
    ...
    if(u.origin===location.origin && ['/python','/trial'].includes(u.pathname) && (u.hash==='#apply'||u.pathname==='/trial')) {
      gtag('event','trial_link_click',{...params(),destination:u.pathname});
    }
  });
