...
실행 중 CSV를 교체·이동·삭제했다면 실행 시점/직전 저장본과 비교해 충돌을 알리고 덮어쓰지 않는다. 다른 프로젝트나 `.py`/이미지 파일은 런타임 저장 메시지로 수정할 수 없다. 저장 공간 부족 및 다른 탭 충돌은 기존 초안 오류 안내로 처리한다. 과제 제출의 ‘게임 스튜디오에서 추가’는 이전 요청대로 `.py`만 선택한다.
