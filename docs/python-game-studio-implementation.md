...
- ‘내 프로젝트 → 몬스터 잡기 예제’는 빈 `main.py`와 작은 기본 이미지/효과음만 제공한다. 학생이 완성 코드를 바로 보지 않고 에셋을 사용해 직접 작성한다. 원본 에셋을 재현한 그림은 아니다.
...
- `public/python-game-examples`의 PNG는 코드로 그린 단순 도형, OGG는 합성 사인파다. `monster-main.py`는 기존 검증·참고 자료로 남아 있지만 학생용 몬스터 스타터에는 삽입되지 않는다.
