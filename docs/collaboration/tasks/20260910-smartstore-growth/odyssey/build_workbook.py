...
OUT=ROOT/'output/pdf/odyssey-reading-workbook-review.pdf'
...
C.setTitle('오뒷세이아 원작 읽기 워크북 - 18강 수강 보조자료 검토본')
C.setAuthor('둘시네 상품기획 / 수강 보조자료 검토본')
page=0
...
def base(kicker,title,sub=''):
    ...
    text('둘시네 | 오뒷세이아 원작 읽기 | 검토본 2026.09',48,31,8,color=MUTED)
...
para('수강 보조자료 시안입니다. 영상의 발언을 옮긴 녹취록이나 강사의 확정 해설집이 아닙니다. 실제 강의와 대조한 뒤 최종 배포본을 확정합니다.',48,143,size=10,color=MUTED,leading=17)
finish()
...
for offset in (0,9):
    ...
    y=665
    text('강',48,y,10,color=TEAL);text('읽을 범위 / 이번 강의의 관점',92,y,10,color=TEAL);text('재생 시간',468,y,10,color=TEAL)
...
base('SOURCES & NEXT STEP','대조 자료와 최종 검토','읽기의 근거를 남기고, 강의의 고유한 설명으로 보강하기')
y=665
for title,body in [
...
('배포 전 확인','강의의 실제 설명과 각 질문의 연결, 사용 번역본의 행 번호, 인물 이름 표기를 강사가 확인해야 합니다. 본 검토본의 질문은 강사의 실제 발언이나 정답을 대신하지 않습니다.')]:
...
para('자료 구성: 이용 안내 1쪽, 강의 목차 2쪽, 서사 구조·인물 2쪽, 강의별 워크시트 18쪽, 영화 비교 1쪽, 대조 자료 1쪽과 표지. 이메일 배포용 PDF 시안.',48,113,size=9,leading=15,color=MUTED)
finish()
C.save()
print(f'{OUT}\npages={page}')
