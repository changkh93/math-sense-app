...
| 분수 겹침 렌즈 | `equivalent_fractions` | 같은 크기 분수 탐구 1개 완료 | 10광석 |
| 통분 겹침 렌즈 | `common_denominator` | 통분·분수 비교 탐구 1개 완료 | 10광석 |
| 약분 묶음 연구소 | `fraction_reduction` | 약분 탐구를 기약분수까지 완료 | 10광석 |
...
- 카드 수, 선택한 단, 오답 횟수, 미션 번호, 관찰한 분수, 통분한 공통분모, 약분 단계 수 같은 `metrics`는 피드백 근거용 요약이며 보상액을 바꾸지 않는다.
...
- 같은 체험 활동의 여러 완료 기록은 한 카드에 묶어 “오늘 N개 완료, +총 지급 광석”으로 보여 줄 수 있다. 완료 수와 광석 수가 같다고 가정하지 않는다.
...
- 카드 정답, 세로셈 미션 완료, 같은 크기 분수·통분·약분 탐구 완료는 구체적인 연습 근거다. 활동 제목과 완료 수를 칭찬할 수 있지만, 화면 진입·렌즈 이동·중간 로컬 저장만으로 완료했다고 단정하지 않는다.
- 같은 크기 분수는 정답 분자·분모를 모두 확인한 시점, 통분은 두 분수를 통분하고 크기 비교까지 맞힌 시점, 약분은 여러 단계가 필요하더라도 기약분수에 도달한 시점만 완료로 기록한다.
...
- 분수 피드백 문구 예: “약분 묶음 연구소 탐구 2개를 기약분수까지 완성해 분자와 분모를 같은 공약수로 나누는 과정을 직접 연습했습니다.”
...
- 카드 정답, 세로셈 완료, 분수 탐구 최종 완료 직후 저장 중·광석 획득·이미 받은 항목·저장 실패 상태가 학생 눈높이로 표시되는지 확인한다.
...
`suggestedBonusCrystals`는 10~40 사이로 준다. 보너스 광석 지급은 지나치게 인색하거나 경직되지 않고 **어느 정도 후하고 공정하게** 부여하여 학생의 지속적인 학습 동기와 성취감을 북돋운다. 단, 무조건적인 남발이 아니라 성실성과 질적 완성도에 부합하도록 아래 **구간별 권장 비율 가이드**를 준수한다.
...
### 권장 광석 구간별 비율 가이드 (전체 제출 대비)

| 광석 | 권장 비율 | 기준 및 판단 방식 |
| :---: | :---: | --- |
| **40** | **상위 약 10%** | **최우수 과제 (만점)**: 기준 학습량 충족, 기록 구체성, 집중도 광석 완벽 획득, 자발적 질문/오답 성찰, CODE TRACE 완주 또는 고전 독서 40점 루브릭 만점 등 모든 면에서 흠잡을 데 없이 탁월한 과제. (전체 과제 중 상위 10% 내외로 엄선하되 결코 만점이 0건이 되지 않도록 유의) |
| **35** | **약 20~25%** | **매우 우수**: 퀴즈 다수 만점, 다크매터 대량 정화, CODE TRACE/Lumi 완료, 적극적 질문이나 깊이 있는 취약점 성찰, 높은 영상 몰입도(집중도 만점) 등 매우 모범적이고 균형 잡힌 학습 |
| **30** | **약 25~30%** | **표준 성실 (중심 구간)**: 영상 시청 + 집중도 광석 획득 + 퀴즈 및 기본 실습을 성실히 수행하여 당일 학습 흐름이 안정적인 경우. Python 과제에서 영상이 짧아도 CODE TRACE 또는 LUMI를 의미 있게 진행하고 제출문이 해당 구조·상태 변화를 설명한 경우 |
| **25** | **약 20% 내외** | **보통/노력 인정**: 스마트 워크북 보충 풀이, 퀴즈 도전, 부분 실습 진행 등 일부 확인 활동이 부족하거나 제출문이 짧지만 성실하게 학습을 이어간 경우 |
| **20** | **약 10~15%** | **기본 제출**: 외부 문제풀이/독서 진행 짧은 1줄, 자기 점검 복습 시도 등 플랫폼 확인 활동은 거의 없으나 과제 제출 사실 자체가 유효하고 노력한 흔적이 있는 경우 |
| **10~15** | **약 5~10% 미만** | **학습량 극히 부족/불일치**: 플랫폼 학습 기록이 0건이고 제출 내용도 1줄 단편적이거나 성의가 극히 부족한 경우에 한정 |

### 공정하고 후한 보너스 산정을 위한 핵심 원칙

- **인색함 방지 (단일 지표 잣대 금지)**: 영상 시간이 50분 기준에 못 미치더라도 퀴즈 만점이 여러 개이거나, 다크매터 오답을 대량으로 정화했거나, CODE TRACE/Lumi 실습을 완료했다면 20광석 이하로 깎지 않고 **30~35광석**을 적극 부여한다.
- **자발적 질문 및 오답 성찰 우대**: 스스로 모르는 수학 개념을 질문하거나, 코딩 중 발생한 오류를 질문하거나, 헷갈렸던 오답의 이유를 솔직하고 깊이 있게 성찰한 제출에는 적극적으로 상위 구간(35~40광석)을 부여한다.
- **고전 독서의 질적 평가**: 단순 쪽수 경쟁이 아니라, 인물의 심리/행동 동기 분석, 전편과의 서사 연결, 등장인물 비판 등 깊이 있는 독서 감상이 확인되면 과감하게 **35~40광석**을 부여한다.
- **만점(40광석) 비율 준수**: 전체 제출 중 약 10%(50건 기준 5~6건)는 학습과 제출 퀄리티가 가장 뛰어난 학생들에게 확실하게 만점을 지급하여 최상위 성취에 대한 확실한 보상을 제공한다.
