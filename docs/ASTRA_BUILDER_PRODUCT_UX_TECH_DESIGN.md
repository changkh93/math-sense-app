...
> 재료 라이브러리, 퀵바·재료함, 재질 변경, 영구 레시피 ID,
> `u32le-v2` 마이그레이션의 현재 승인 계획은
> `docs/ASTRA_BUILDER_MATERIAL_RECIPE_UX_PLAN.md`를 따른다.
...
### 2026-07-24 · 행성 객체 Stage 2와 월드 상호작용 보강

완료:

- 모든 건설 카탈로그 객체에 공통 `level`, `maxLevel`, `stage2Cost`,
  `stage2Available` 계약 추가
- 첫 대표 자산으로 루멘 나무 Stage 2 구현
  - 굵기 변화가 있는 줄기와 뿌리
  - 네 방향 가지, 7개 수관 군집, 발광 열매와 덩굴
  - 건설 설계소와 설치 객체 정보창에서 동일한 실시간 3D 미리보기 사용
- 루멘 나무는 Stage 1 기본 건설 60광석, Stage 2 직접 건설은
  성장 비용 45광석을 합산한 105광석으로 표시·결제
- 이미 설치된 객체는 `F → 정보 → 수정 → GRAPHIC STAGE`에서 성장 가능
- 신규 `upgradeGalaxyItem` callable에서 소유권, 현재 등급, 준비된 자산,
  보유 광석과 중복 operation을 트랜잭션으로 검증
- 아직 그래픽이 준비되지 않은 객체는 같은 UI를 표시하되
  `Stage 2 준비 중`으로 비활성화
- 큰 시설은 충돌 반경을 고려해 외곽에서도 F 정보가 닿도록 상호작용 거리 보정
- 행성 지형의 장식 나무·바위·기계물에도 이동 충돌체 추가
- 아스트라 빌더 부지 외곽에 진입 경계를 두고, 경계 밖에서 안내와 E 진입이
  먼저 나타나도록 감지 반경 확대

실브라우저 QA:

- 루멘 나무 Stage 1/2 전환과 3D 미리보기, 60/105광석 비용 표시 확인
- 설치된 수정 연못에서 F 정보창과 수정 화면의 공통 Stage UI 확인
- 부지 밖 길 위에서 `아스트라 빌더 시작` 안내 노출 및 E 진입 확인
- 기존 빌더 서버 상태 268블록을 수정하지 않고 정상 로드 확인
- 실제 계정 광석 보존을 위해 최종 업그레이드 결제 버튼은 누르지 않음
